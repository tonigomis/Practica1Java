public class Transposition {
    static String cypher(String s, int dim) {
        ...
        return ompleXifrat(codifica, resultat);
    }
    ...
    static String decypher(String s, int dim) {
        ...
        for (int i = 0; i < decodifica[0].length; i++) {
            for (int j = 0; j < decodifica.length; j++) {
                if (!(j == decodifica.length - 1 && i > (decodifica[0].length - 1) - (decodifica.length * decodifica[0].length - s.length())) && t < s.length()) {
                    decodifica[j][i] = s.charAt(t);
                    t++;
                } else {
                    decodifica[j][i] = '\u0000';
                }
        ...
        return ompleDesxifrat(decodifica, resultat);
    }
    ...
    static String ompleXifrat(char[][] codifica, String resultat) {
    ...
    static String ompleDesxifrat(char[][] decodifica, String resultat) {
